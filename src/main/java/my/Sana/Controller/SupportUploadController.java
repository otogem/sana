...
@Controller
public class SupportUploadController {
	...
	@PostMapping("/spdeleteFile")
	@ResponseBody
	public ResponseEntity<String> spdeleteFile(String fileName, String type){
		File file;
		try {
			file = new File("D:\\spupload\\" + URLDecoder.decode(fileName, "UTF-8"));
			file.delete();
	...
	@RequestMapping(value="/SupportuploadAjaxAction",method=RequestMethod.POST)
	public ResponseEntity<ArrayList<SupportFileVO>> uploadAjaxPost(MultipartFile[] uploadFile) {
		
		//ServiceFileListVO클래스 포함
		ArrayList<SupportFileVO> list = new ArrayList();
		
		//폴더경로
		String uploadFolder="D:\\spupload";
		...
		//                     어느폴더에(D:\\upload  ,  어떤파일이름으로(UUID_mainlogo_new.png)
	...
	// 이미지 주소 생성
	@RequestMapping(value="/supportdisplay",method=RequestMethod.GET)
	public ResponseEntity<byte[]> getFile(String fileName){
		System.out.println(fileName);
		
		File file = new File("D:\\spupload\\"+fileName);
	...
	//다운로드 주소 생성
	@RequestMapping(value="/supportdownload",method=RequestMethod.GET)
	public ResponseEntity<Resource> downloadFile(String fileName){
		
		Resource resource=new FileSystemResource("D:\\spupload\\"+fileName);
