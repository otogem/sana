...
$(document).ready(function(){
	...
	$("input[type='file']").change(function(e){
		...
		//ajax를 통해서 PostUploadController에 파일 관련 데이터 전송.
		$.ajax({
			type:"post",
			url:"/PostuploadAjaxAction",
			data:formData,
			contentType:false,
			processData:false,
			dataType:"json",
	...
	var uploadResult = $("#uploadResult ul");	
		...
		//글쓰기 버튼을 클릭했을 때
				...
				$("#uploadBtn").on("click",function(e){
					...
					var str="";
					var empty='';
					if($("input[name='product_name']").val().trim()==empty||$("input[name='product_name']").val().trim()==null){
						alert("상품을 입력하세요.");
						$("input[name='product_name']").val(empty);
						return false;
					}else if($("textarea[name='product_desc']").val().trim()==empty||$("textarea[name='product_desc']").val().trim()==null){
						alert("제품 상세내용 을 입력하세요.");
						$("textarea[name='product_desc']").val(empty);
						return false;
					}
					...
					$("#uploadResult ul li").each(function(i,file){
						...
						str += "<input type = 'hidden' name = 'postfile["+i+"].fileName' value = '" + jfile.data("filename")+"'>";
						str += "<input type = 'hidden' name = 'postfile["+i+"].uuid' value = '" + jfile.data("uuid") + "'>";
						str += "<input type = 'hidden' name = 'postfile["+i+"].uploadPath' value = '" + jfile.data("path") + "'>";
						str += "<input type = 'hidden' name = 'postfile["+i+"].image' value = '" + jfile.data("type") + "'>";
					})
